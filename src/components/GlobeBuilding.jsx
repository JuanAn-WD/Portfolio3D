...
const GlobeBuilding = React.forwardRef(({
  ...
  useFrame(() => {
    ...
    interactRef.current.getWorldPosition(pos)
    const isClose = pos.length() < interactDistance
      ...
      <group position={[0, 50 + position[1], 0]}>
        <group rotation={rotation} ref={ref}>
          ...
          <Html position={[0, popupHeight, 0]} center style={{ pointerEvents: 'none' }}>
            <div style={{ backgroundColor: 'rgba(15, 23, 42, 0.85)', backdropFilter: 'blur(8px)', color: '#60a5fa', padding: '8px 16px', borderRadius: '12px', fontFamily: 'system-ui, sans-serif', fontWeight: '800', fontSize: '14px', whiteSpace: 'nowrap', border: '2px solid rgba(96, 165, 250, 0.3)', boxShadow: '0 8px 20px rgba(0,0,0,0.4)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              {popupTitle}
            </div>
          </Html>
        </group>
